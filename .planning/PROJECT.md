...
- **Current state (after Phase 04.1 gap closure, 2026-09-15):** Phase 4's baked-response roughness extraction (Sobel parity + fit-driven strength search gated on FitOnlyMaxError), vertex-color decomposition with residual + one-texture collapse, and the D-06 roughness-offset input are delivered and gap-closed. Reprocess is now idempotent end-to-end: materials recover their source via NamerSource tags AND dedupe on the resolved source (source + its generated material on one hierarchy count as one), and generated mesh assets overwrite in place so scene renderers never lose their reference (the 2nd-Press vanish). The processor window's preview mirrors the CR-01 decomposition-skip predicate with a visible reason, and pooled render textures unbind RenderTexture.active before release. Code-review dispositions resolved (04.1-REVIEW.md); EditMode 120/120 verified in the live editor, including double-Process reprocess, live-EditorPrefs isolation pins, and resolved-source dedupe regressions. Remaining: two human visual checks (fit-driven debug channel / shaded preview + the Sobel-glossy Blender-parity design decision; Neo double-Process confirmation) and the 1Password-blocked commit batch.
